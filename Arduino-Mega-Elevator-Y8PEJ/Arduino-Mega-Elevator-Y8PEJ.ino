...
#define PULLEY_RADIUS 12.16
#define BELT_PITCH_RADIUS 12.74
#define PULLER_CIRCUMFERENCE (2*BELT_PITCH_RADIUS)*PI
...
const int safetyFactorOffset = flagDetectionSteps * 2; // * 2;
...
enum state {
  ...
  elevatorLimitTrigg = 0x63,
  stopped = 0x52,
  safetySwitchLower = 0x62,
  safetySwitchUpper = 0x61,
  parameter = 0x70
};
...
void loop()
{
  ...
  switch (mainCommand)
  {
        ...
        case safetyMoving:
          ...
          else if (!counterExceeded())
          {
            ...
            if (!safetyBarrier())
            {
              if (debug == 2)
              {
                Serial.println("SafetyBarrier exit'ed");
              }
   ...
   //-------------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------------------------------------
    //No command is recognized. The arduino should never reside in this case..
    default:
      if (debug == 3)
      {
